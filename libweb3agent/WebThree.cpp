...
WebThreeDirect::WebThreeDirect(std::string const& _clientVersion,
    ...
    CI::cic::ChainParams const& _params, WithExisting _we, std::set<std::string> const& _interfaces,
  ...
  : m_clientVersion(_clientVersion), m_net(_clientVersion, _n, _network)
{
    ...
    if (_interfaces.count("eth"))
    {
        ...
        if (_params.sealEngineName == "Ethash")
            m_ethereum.reset(new CI::cic::EthashClient(_params, (int)_params.networkID, &m_net, shared_ptr<GasPricer>(), _dbPath, _snapshotPath, _we));
            ...
            m_ethereum.reset(new CI::cic::Client(_params, (int)_params.networkID, &m_net, shared_ptr<GasPricer>(), _dbPath, _snapshotPath, _we));
...
CI::cnet::NetworkPreferences const& WebThreeDirect::networkPreferences() const
{
...
//FIXME
/*
void WebThreeDirect::setNetworkPreferences(CI::cnet::NetworkPreferences const& _n, bool _dropPeers)
{
