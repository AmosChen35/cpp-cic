...
void Host::startedWorking()
{
    ...
    if (port > 0)
    {
        ...
        LOG(m_logger) << "cnet.start.notice id: " << id() << " TCP Listen port is invalid or unavailable.";
    ...
    */
    ...
    LOG(m_logger) << "cnet.started id: " << id();
...
bool Host::addNodeToNodeTable(Node const& _node, NodeTable::NodeRelation _relation)
{
    auto nodeTable = this->nodeTable();
    if (!nodeTable)
        return false;
