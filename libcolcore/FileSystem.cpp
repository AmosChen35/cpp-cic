...
void setDataDir(fs::path const& _dataDir)
{
    s_ethereumDatadir = _dataDir;
}

void setIpcPath(fs::path const& _ipcDir)
{
...
fs::path getDataDir(string _prefix)
{
    if (_prefix.empty())
        _prefix = "ethereum";
...
fs::path getDefaultDataDir(string _prefix)
{
    if (_prefix.empty())
        _prefix = "ethereum";
...
fs::path appendToFilename(fs::path const& _orig, string const& _suffix)
{
