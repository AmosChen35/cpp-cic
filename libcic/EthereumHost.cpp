...
public:
    ...
    void onPeerNodeData(std::shared_ptr<EthereumPeer>, RLP const& _r) override
    {
        unsigned itemCount = _r.itemCount();
        LOG(m_logger) << "Node Data (" << dec << itemCount << " entries)";
    }

    void onPeerReceipts(std::shared_ptr<EthereumPeer>, RLP const& _r) override
    {
        unsigned itemCount = _r.itemCount();
...
EthereumHost::EthereumHost(BlockChain const& _ch, OverlayDB const& _db, TransactionQueue& _tq, BlockQueue& _bq, u256 _networkId):
    //FIXME HostCapability<EthereumPeer>(),
    Worker      ("ethsync"),
    m_chain     (_ch),
    m_db(_db),
    m_tq        (_tq),
    ...
    m_hostData(make_shared<EthereumHostData>(m_chain, m_db))
{
    ...
    */
    ...
    m_tq.onImport([this](ImportResult _ir, h256 const& _h, h512 const& _nodeId) { /* FIXME onTransactionImported(_ir, _h, _nodeId); */ });
}
