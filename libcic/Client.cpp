...
Client::Client(ChainParams const& _params, int _networkID, CI::cnet::Host* _host,
...
void Client::init(CI::cnet::Host* _extNet, fs::path const& _dbPath, fs::path const& _snapshotDownloadPath, WithExisting _forceAction, u256 _networkId)
{
...
pair<h256, Address> Client::submitTransaction(TransactionSkeleton const& _t, Secret const& _secret)
{
    ...
    const u256 defaultTransactionGas = 400000;
