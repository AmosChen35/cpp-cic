...
std::ostream& CI::cic::operator<<(std::ostream& _out, LogFilter const& _s)
{
    // TODO
...
bool LogFilter::matches(LogBloom _bloom) const
{
    if (m_addresses.size())
    {
        for (auto const& i: m_addresses)
            if (_bloom.containsBloom<3>(CI::col::sha3(i)))
                goto OK1;
        return false;
    }
    OK1:
...
vector<LogBloom> LogFilter::bloomPossibilities() const
{
    ...
    for (auto const& i: m_addresses)
    {
        ...
        for (auto const& t: m_topics)
            if (t.size())
            {
                LogBloom b = LogBloom().shiftBloom<3>(CI::col::sha3(i));
                for (auto const &j: t)
                    b = b.shiftBloom<3>(CI::col::sha3(j));
                ret.push_back(b);
            }
    }
    ...
    if (!ret.size())
        for (auto const& i: m_addresses)
            ret.push_back(LogBloom().shiftBloom<3>(CI::col::sha3(i)));
