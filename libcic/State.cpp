...
void State::clearCacheIfTooLarge() const
{
    ...
    while (m_unchangedCacheEntries.size() > 1000)
    {
        ...
        size_t const randomIndex = std::uniform_int_distribution<size_t>(0, m_unchangedCacheEntries.size() - 1)(CI::col::s_fixedHashEngine);
...
void State::commit(CommitBehaviour _commitBehaviour)
{
    if (_commitBehaviour == CommitBehaviour::RemoveEmptyAccounts)
        removeEmptyAccounts();
    m_touched += CI::cic::commit(m_cache, m_state);
    m_changeLog.clear();
    m_cache.clear();
...
template AddressHash CI::cic::commit<OverlayDB>(AccountMap const& _cache, SecureTrieDB<Address, OverlayDB>& _state);
template AddressHash CI::cic::commit<MemoryDB>(AccountMap const& _cache, SecureTrieDB<Address, MemoryDB>& _state);
